...
export default function ServicePageContent({ slug }: ServicePageContentProps) {
    // Determine service from static data
    const service = serviceData.find((s) => s.slug === slug);

    if (!service) {
        // In a client component, we might handle 404 differently or just let the parent handle it
        // But since we are moving logic from the page, let's keep the check.
        // If the parent server component checked it, this might not be reached.
        return notFound();
    }
