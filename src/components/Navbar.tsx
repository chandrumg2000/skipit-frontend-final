            ...
            >
                <div className="mx-auto max-w-7xl px-6 flex items-center justify-between">
                    ...
                    <Link href="/" className="flex items-center gap-2 group">
                        <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/20 transition-transform group-hover:scale-105">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M6 4l10 8-10 8V4z M16 4l0 16 2 0 0-16z" style={{ display: 'none' }} /> {/* Old path backup just in case, but replacing with double triangle */}
                                <path d="M4 5L12 12L4 19V5Z" />
                                <path d="M12 5L20 12L12 19V5Z" />
                            </svg>
                        </div>
                        <span className="text-2xl font-black tracking-tighter text-white lowercase leading-none pb-1">
                    ...
                    <div className="hidden md:flex items-center gap-1">
                        <div className="flex items-center rounded-full border border-white/10 bg-white/5 px-2 py-1.5 backdrop-blur-sm">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.name}
                                    href={link.href}
                    ...
                    <div className="hidden md:block">
                        <Link
                            href="/contact"
                            className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-white px-6 py-2.5 text-sm font-bold text-black transition-transform hover:scale-105 hover:bg-cyan-50"
                        >
                            <span>Get Started</span>
                            <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                            <div className="absolute inset-0 -z-10 bg-gradient-to-r from-cyan-400/20 to-blue-400/20 opacity-0 transition-opacity group-hover:opacity-100" />
                        </Link>
                    </div>
